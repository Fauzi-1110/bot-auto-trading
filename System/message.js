    ...
    try {
        ...
        switch (command) {
            ...
            case "season-new": {
                ...
                fc.replyMsg("Berhasil membuat season baru ("+dbSystem.getGlobalKey("season")+")!")
            } break
            ...
            case "kerja": {
                ...
                if (user.pekerjaan.pekerjaan === "pengangguran") {
                    ...
                    if (now - user_kerja.last_kerja <= data_kerjaan.cooldown)return fc.reply("Kamu sudah bekerja! Coba lagi nanti...")

                    let gaji = 0
                    if (["pedagang", "pembisnis", "ceo"].includes(kerja.pekerjaan)) {
                        const untung = Math.random() > 0.25
                        const close = Math.random() < 0.1
                        gaji = Math.floor(Math.random() * data_kerjaan.gaji)
                        if (!untung) {
                            gaji *= -1
                            fc.reply("Kamu mengalani kerugian hingga mencapai "+gaji)
                        }
                        if (close) {
                            dbSystem.setUserKey(id, "pekerjaan.pekerjaan", "pengangguran")
                            fc.reply("Kamu bangkrut hingga berhenti menjadi "+kerja.pekerjaan)
                        }
                        ...
                        if (dipecat) {
                            dbSystem.setUserKey(id, "pekerjaan.pekerjaan", "pengangguran")
                            fc.reply("Kamu dipecat dari pekerjaan mu!")
                        }
                    }

                    fc.reply("Kamu mendapatkan uang sebanyak "+gaji+" hasil dari kerjamu!")
...
try{
    ...
    switch(command){
        ...
        case "season-new": {
            ...
            //reset dta user
            for(let id of keyUser){
                const listReset = ["perak", "emas", "platinum", "diamond", "balaceCoin", "roadaCoin", "flagCoin", "timenCoin", "loyaliCoin", "jumlah_trading", "total_profit",
                                   "profit.perak", "profit.perak_modal", "profit.emas", "profit.emas_modal", "profit.platinum", "profit.platinum_modal", "profit.diamond", "profit.diamond_modal", 
                                   "profit.balaceCoin", "profit.balaceCoin_modal", "profit.roadaCoin", "profit.roadaCoin_modal", "profit.flagCoin", "profit.flagCoin_modal", "profit.timenCoin", "profit.timenCoin_modal", "profit.loyaliCoin", "profit.loyali_modal", ]
                const user = allUser[id]
                
                dbSystem.setUserKey(id, "uang", 300)
                listReset.forEach((k) => {
                    dbSystem.setUserKey(id, k, 0)
                })
            }
